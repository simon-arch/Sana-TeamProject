...
namespace Server.Data.Repositories
{
    public class UserRepository(DbProvider provider) : IUserRepository
    {
        ...
        private async Task<int> _CountAsync(string condition) =>
            await _sql.QuerySingleAsync<int>($"SELECT COUNT(*) FROM Users {condition}");

        public async Task<User?> GetAsync(string username) => await _GetAsync($"Username = '{username}'");

        public async Task<User?> GetAsync(Guid tokenId) => await _GetAsync($"TokenId = '{tokenId}'");
        ...
        private async Task<User?> _GetAsync(string condition)
        {
            ...
            return await _sql.QueryFirstOrDefaultAsync<User>(query);
        }

        public async Task<ResultSet<User>> GetAllAsync() => await GetAllAsync(new GetAllOptions());
        ...
        {
            string query = @"
                INSERT INTO Users (Username, PasswordHash, TokenId, FirstName, LastName, Role, Permissions, State, WorkType, WorkingTime, ApproveVacationsForUsers, VacationsApprovedByUsers)
                VALUES (@Username, @PasswordHash, @TokenId, @FirstName, @LastName, @Role, @Permissions, @State, @WorkType, @WorkingTime, @ApproveVacationsForUsers, @VacationsApprovedByUsers)";
        ...
        public Task UpdateAsync(User user)
        {
            string query = $@"
                ...
                ApproveVacationsForUsers = @ApproveVacationsForUsers, VacationsApprovedByUsers = @VacationsApprovedByUsers
