...
namespace Server.Data.Repositories
{
    public class VacationRepository(DbProvider provider) : IVacationRepository
    {
        ...
        {
            string query = @"
                SELECT Id, Description, Status, Sender, StartDate, EndDate
                FROM Vacations";
        ...
        {
            string query = @"
                INSERT INTO Vacations (Description, Status, Sender, StartDate, EndDate)
                OUTPUT INSERTED.Id
                VALUES (@Description, @Status, @Sender, @StartDate, @EndDate)";
        ...
        public Task UpdateAsync(Vacation appeal)
        {
            string query = @"
                UPDATE Vacations
                SET Description=@Description, Status=@Status, Sender=@Sender, StartDate=@StartDate, EndDate=@EndDate
                WHERE Id=@Id";
